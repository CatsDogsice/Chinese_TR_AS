...
def wrapclass(cls):
    """Wrap AutoModelForTokenClassification to support CRF and other functions.进一步包装使得支持CRF和其他功能
    @type cls: object
    """
    class NERAddon(cls):
        ...
        def forward(
            ...
            embeddings = torch.cat(cat_features, dim=1)
            ...
            if self.use_crf:#F
                ...
                for i, tag in enumerate(tags):#tag是一个元组
                    for j, t in enumerate(tag[0]):
                        p_logits[i, idx[i][j], t] = tag[1]
                     #tag_sequence, viterbi_score
                if labels is not None:
            ...
            if not return_dict:
                if "nll" in self.return_list:
                    ...
                    output = outputs[2:]
                output = (logits, sequence_output) + output
...
class NERModel(AutoModelForTokenClassification):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if not isinstance(config, PretrainedConfig):
            print("isinstance(config, PretrainedConfig:)", isinstance(config, PretrainedConfig))
            config, kwargs = AutoConfig.from_pretrained(
                pretrained_model_name_or_path, return_unused_kwargs=True, **kwargs
            )

        for config_class, model_class in MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING.items():
            if isinstance(config, config_class):#True
                print("isinstance(config, config_class):", isinstance(config, config_class))
