    ...
    with open(file_path, 'r') as f:
        ...
        for item in data:
            ...
            for i,l in enumerate(labels):
                labels_ids.append(tag_to_ids[labels[i]])#

            if "hp_label" in item:
                hp_labels = item["hp_label"]
                for i, l in enumerate(words):
                    hp_labels_id.append(tag_to_ids[hp_labels[i]])

            else:
                hp_labels_id = [None]*len(labels)

            examples.append(InputExample(guid="%s-%d".format(mode, guid_index), words=words, labels=labels_ids, hp_labels=hp_labels_id))#CHEM英文数据labels=labels_ids,hp_labels=hp_labels#中文labels=labels_ids, hp_labels=hp_labels_id
            guid_index += 1
            labels_ids = []
            hp_labels_id = []
