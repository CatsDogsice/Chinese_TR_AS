...
class LDA:
    ...
    def __init__(self, texts, num_topics=None, start=5, limit=35, step=5, print_summary=True, workers=1):
        """
        LDA 模型
    ...
    def compute_coherence_values(self, limit, start=5, step=5):#3
        """
        ...
        for num_topics in range(start, limit, step):
            model = gensim.models.wrappers.LdaMallet(
                mallet_path, corpus=self.corpus, num_topics=num_topics, id2word=self.id2word, workers=self.workers)
    ...
    def train(self):
        if self.num_topics:
            self.best_model = gensim.models.wrappers.LdaMallet(
                mallet_path, corpus=self.corpus, num_topics=self.num_topics,
            ...
            model_list, coherence_values = self.compute_coherence_values(start=5, limit=35, step=5)
            x = range(self.start, self.limit, self.step)
            draw_train_result(x, coherence_values)
    ...
    def format_topics_sentences(self):
        ...
        sent_topics_df.to_csv('sent_topics_df.csv')
