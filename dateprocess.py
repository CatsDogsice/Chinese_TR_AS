...
class RoSTERUtils(object):
    ...
    def read_file(self, file_dir="conll", dataset_name="train", supervision="dist"):
        ...
        for text_line, label_line in zip(text_contents, label_contents):
            sentence = text_line.strip().split()[0]
    ...
    def read_types(self, ffile_path):
    ...
    def get_tensor(self, label_map, dataset_name, max_seq_length, supervision="true", drop_o_ratio=0 ):
        ...
        if os.path.exists(data_file):
            ...
            for text, labels in tqdm(all_data, desc="Converting to tensors"):
                ...
                for i, token in enumerate(tokens[1:], start=1):
                    ...
                    if i == 1 or i < max_seq_length - 1:
                        label = labels[j]
                        label_idx[i] = label_map[label]
                        valid_pos[i] = 1
                        j += 1
